...
class Test_ttpassgen(unittest.TestCase):
    ...
    def test_invalid_options(self):
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$0', mode=233), 0)

        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go(dictlist=None, rule=None), 0)

        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('[?d]', repeatMode="cc9"), 0)

        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$0word233', dictlist=''), 0)

        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$99[hello]'), 0)

        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$0', dictlist='not_exist.dict'), 0)
        ...
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('[?d]{0}'), 0)

    def test_not_exist_output_file(self):
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$0'), 6)

        # check not exist directory.
        if os.path.exists('tempDir'):
            shutil.rmtree('tempDir')
        self.assertEqual(go('$0', output='tempDir/testout.dict'), 6)
        if os.path.exists('tempDir'):
            shutil.rmtree('tempDir')
    ...
    def test_word_separator(self):
        self.assertEqual(go('$0[abc]?', separator=' '), 1)
        self.assertEqual(
            go('$0[abc]?', separator='-------------------------\n'), 24)
    ...
    def test_char_array_estimated_size(self):
        ...
        if os.name == 'nt':
            self.assertEqual(ttpassgen.pretty_size(size), "4.61 MB")
        else:
            self.assertEqual(ttpassgen.pretty_size(size), "4.84 MB")
    ...
    def test_part_size_with_complex_rule(self):
        ...
        if os.path.exists('testout.dict.3'):
            os.remove('testout.dict.3')
        self.assertEqual(total_line, 199980)

    def test_multiprocessing_complex_rule(self):
        self.assertEqual(go('[789]{0:3:*}$0[?q]$0', debugMode=0), 1440)
        self.assertEqual(go('[789]{0:3:*}$0[?q]{1:?}$0', debugMode=0), 1440)

    def test_dick_cache(self):
        self.assertEqual(go('$0[abc]?', diskCache=0), 24)
    ...
    def test_part_size(self):
        ...
        if os.path.exists('testout.dict.3'):
            os.remove('testout.dict.3')
        self.assertEqual(total_line, 375076)

    def test_wrong_length_in_char_array(self):
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('[789]{0:-3}'), 0)

    def test_max_length_greater_than_char_array_size(self):
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('[789]{2:5}'), 0)

    def test_only_normal_string(self):
        self.assertEqual(go('abc'), 1)

    def test_string_array(self):
        self.assertEqual(go('$(123,xx,789){2:2:?}'), 6)
        self.assertEqual(go('$(123,xx,789){2:2:*}'), 9)

    def normal_string_with_char_array(self):
        self.assertEqual(go('abc[123]{1:2}'), 9)
        self.assertEqual(go('[123]{1:2}abc'), 9)
        self.assertEqual(go('[123]{1:2}abc[45]'), 18)

    def test_char_array_with_string_array(self):
        self.assertEqual(go('aa$(123,456){1:2:?}bb[xy]cc'), 8)

    def test_dict_with_string_array(self):
        self.assertEqual(go('$0[A]?xy'), 12)

    def test_wrong_length_in_string_array(self):
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$(123,456){3:2:?}'), 0)

    def test_max_length_greater_than_string_array_size(self):
        if os.path.exists('testout.dict'):
            os.remove('testout.dict')
        self.assertEqual(go('$(123,456){3:4:?}'), 0)
    ...
    def test_dict_content_not_end_with_line_separator(self):
        ...
        with open('in3.dict', 'a') as f:
            f.write('end')
        self.assertEqual(go('$1eng', dictlist=dictlist), 7)
